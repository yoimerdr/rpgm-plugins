...
### Fixed
- Fixed mapper conversion functions (`fromSourceTransform`, `toSourceTransform`, `flattenobj`) to strictly validate types and prevent runtime crashes.
- Prevented silent crashes when loading translations with missing or incomplete collections (such as `images`, `maps`, `commonEvents`).
- Added boundary and type validation in `PluginHandler.load()` to gracefully handle empty language configurations.
- Added parameter guards to `TextCommand.split()` and `LanguageOption.equals()` to protect against null/undefined arguments.
